...
snow::clusterExport(cl, list("extract_results", "inv.logit", "grand_mean", "extract_results_lme4"), envir = environment())

## loop over different numbers of data points in each level

for(n_each in c(200, 25, 50, 100, 500)) {
  ...
  for(sd_re in c(0.1, 0.01, 0.5, 2.0)) {
    ...
    snow::clusterExport(cl, list("sd_re"), envir = environment())
      ...
      saveRDS(result_list, file = paste0("Results/results_mountain_glmm_", sd_re, "_",n_each, "_unbalanced.Rds"))
  }
}
