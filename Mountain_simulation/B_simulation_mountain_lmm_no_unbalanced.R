...
for(n_each in c(50, 100, 200, 500)) {
  ...
  for(sd_re in c(0.1, 0.01, 0.5, 2.0)) {
      ...
      result_list = 
        snow::parLapply(cl, 2:8, function(number_groups)  {
          ...
          for(experiment in 1:number_experiments){
            ...
            # glmmTMB - MLE
            try({
              fit_glmmTMB <- glmmTMB::glmmTMB(y ~ x  + (1|group) + (0+x | group), REML = FALSE) 
            ...
            # glmmTMB - REML
            try({
              fit_glmmTMB <- glmmTMB::glmmTMB(y ~ x  + (1|group) + (0+x | group), REML = TRUE)
              summ = summary(fit_glmmTMB)
            ...
            try({
              ...
              results_w_lm[experiment, 1:4] = grand_mean(fit_lm, n_groups, beta = beta)
              results_w_lm[experiment, 5:8] = grand_mean(fit_lm, n_groups, beta = beta0, intercept = TRUE)
            }, silent = TRUE)
            ...
            try({
              fit_lm = lm(y ~ 0+x*group-x)
              results_wo_lm[experiment, 1:4] = grand_mean(fit_lm, n_groups, beta = beta)
              results_wo_lm[experiment, 5:8] = grand_mean(fit_lm, n_groups, beta = beta0, intercept = TRUE)
            }, silent = TRUE)
    ...
    saveRDS(result_list, file = paste0("Results/results_mountain_lmm_no_cov_", sd_re, "_",n_each, "_unbalanced.Rds"))
