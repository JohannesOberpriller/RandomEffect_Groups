...
snow::clusterExport(cl, list("extract_results", "inv.logit", "extract_results_lme4"), envir = environment())
...
for(n_each in c(25, 50, 100, 200, 500)) {
      ...
      snow::parLapply(cl, 2:8, function(number_groups)  {
        ...
        for(experiment in 1:number_experiments){
          ...
          try({
            ...
            results_w_lm[experiment, ] = c(summ$coefficients["x", 1], summ$coefficients["x", 4], 
  ...
  saveRDS(result_list, file = paste0("Results/results_mountain_glmm_no_cov_",n_each,"_.Rds"))
}
