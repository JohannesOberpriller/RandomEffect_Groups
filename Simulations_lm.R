...
snow::clusterEvalQ(cl, { number_experiments = 5000})
...
result_list = 
  snow::parLapply(cl, 2:8, function(number_groups)  {
    ...
    for(experiment in 1:number_experiments){
      ...
      results_w[experiment, 1]= summary(fit_lm)$coefficients["(Intercept)","Estimate"] + (1/n_groups)*sum(summary(fit_lm)$coefficients[-(1:2),"Estimate"])
      ...
      results_w[experiment, 6]= mean(  apply(confs_wo, 1, function(co) as.integer(beta > co[1] & beta < co[2]))  )
  ...
  result_list_slope = 
    snow::parLapply(cl, 2:8, function(number_groups)  {
      ...
      for(experiment in 1:number_experiments){
        ...
        results_w[experiment,6] = mean(  apply(confs_wo, 1, function(co) as.integer(beta0 > co[1] & beta0 < co[2]))  )
        results_w[experiment,7] = mean(fit_summary[,"Pr(>|t|)"] < 0.05)
        ...
        fit_lm = lm(y ~ x:group)
        confs = confint(fit_lm)
        results_wo[experiment,1] = summary(fit_lm)$coefficients["(Intercept)","Estimate"]
        results_wo[experiment,3] = summary(fit_lm)$coefficients["(Intercept)","Pr(>|t|)"]
        results_wo[experiment,5] = as.integer(beta0 > confs["(Intercept)",1] & beta0 < confs["(Intercept)",2])
        fit_summary = summary(fit_lm)$coefficients[-which("(Intercept)" == rownames(summary(fit_lm)$coefficients)),]
        confs_wo = confs[-which(rownames(confs) == "(Intercept)"),]
        results_wo[experiment,6] = mean(  apply(confs_wo, 1, function(co) as.integer(beta0 > co[1] & beta0 < co[2]))  )
        results_wo[experiment,7] = mean(fit_summary[,"Pr(>|t|)"] < 0.05)
  ...
  result_list = 
    snow::parLapply(cl, 2:8, function(number_groups)  {
      ...
      for(experiment in 1:number_experiments){
        ...
        results_w[experiment,1] = summary(fit_lm)$coefficients["(Intercept)","Estimate"] + (1/n_groups)*sum(summary(fit_lm)$coefficients[-(1:2),"Estimate"])  
        results_w[experiment,2] = summary(fit_lm)$coefficients["x","Estimate"]
        results_w[experiment,3] = summary(fit_lm)$coefficients["(Intercept)","Pr(>|z|)"]
        results_w[experiment,4] = summary(fit_lm)$coefficients["x","Pr(>|z|)"]
        results_w[experiment,5] = as.integer(beta > confs["x",1] & beta < confs["x",2])
        fit_summary = summary(fit_lm)$coefficients[-which("x" == rownames(summary(fit_lm)$coefficients)),]
        confs_wo = confs[-which(rownames(confs) == "x"),]
        results_w[experiment,6] = mean(  apply(confs_wo, 1, function(co) as.integer(beta > co[1] & beta < co[2]))  )
        results_w[experiment,7] = mean(fit_summary[,"Pr(>|z|)"] < 0.05)
        ...
        fit_lm = glm(y ~ x  + group, family = binomial)
        confs = confint(fit_lm)
        results_wo[experiment,1] = summary(fit_lm)$coefficients["(Intercept)","Estimate"] + (1/n_groups)*sum(summary(fit_lm)$coefficients[-(1:2),"Estimate"])  
        results_wo[experiment,2] = summary(fit_lm)$coefficients["x","Estimate"]
        results_wo[experiment,3] = summary(fit_lm)$coefficients["(Intercept)","Pr(>|z|)"]
        ...
        fit_summary = summary(fit_lm)$coefficients[-which("x" == rownames(summary(fit_lm)$coefficients)),]
        confs_wo = confs[-which(rownames(confs) == "x"),]
        results_wo[experiment,6] = mean(  apply(confs_wo, 1, function(co) as.integer(beta > co[1] & beta < co[2]))  )
        results_wo[experiment,7] = mean(fit_summary[,"Pr(>|z|)"] < 0.05)
